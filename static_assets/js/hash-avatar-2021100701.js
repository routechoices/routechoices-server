...
!(function () {
  ...
  var t = (function t(e) {
    if (!(this instanceof t)) return new t(f);
    var o,
      a,
      u,
      i,
    ...
    var s = function () {
      ...
      m = function (n, t) {
        if ((1 == arguments.length && ((t = n), (n = 0)), !r(t) || !r(n)))
          throw new Error("parameters must be integers");
        if (t < n) throw new Error("end must be more than start");
        var e = t - n,
          a = s() / o;
        return parseInt(n + Math.floor(a * e), 10);
      };
    return {
      random: h,
      randInt: s,
      randRange: m,
      randFloatRange: function (r, t) {
        if ((1 == arguments.length && ((t = r), (r = 0)), !n(t) || !n(r)))
          throw new Error("parameters must be numbers");
        if (t < r) throw new Error("end must be more than start");
        var e = t - r,
          a = s() / o;
        return parseFloat(r + a * e);
      },
      randChoice: function (r) {
        if (!r instanceof Array) throw new Error("parameters must an array");
        if (!r.length) throw new Error("empty array");
        return r[m(0, r.length)];
      },
      randBool: function (r) {
        return null == r && (r = 0.5), h() < r;
      },
      seed: f,
    };
  })();
...
function hashAvatar(uid, size) {
  ...
  simpleRNG.seed(uid);
  var n = hashCode(uid);
  var c1 = tinycolor({ h: simpleRNG.randRange(360), s: 0.95, l: 0.5 });
  var c1_ = c1.toHexString();
  var c2 = c1.tetrad()[simpleRNG.randRange(1, 4)].toHexString();
  var rot = simpleRNG.randRange(360) * (Math.PI / 180);
  var x1 = Math.round(50 + Math.sin(rot) * 50);
  var y1 = Math.round(50 + Math.cos(rot) * 50);
  var x2 = Math.round(50 + Math.sin(rot + Math.PI) * 50);
  var y2 = Math.round(50 + Math.cos(rot + Math.PI) * 50);
  return `<svg class="rounded-circle" width="${size}px" height="${size}px" viewBox="0 0 80 80" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <defs>
        <linearGradient x1="${x1}%" y1="${y1}%" x2="${x2}%" y2="${y2}%" id="g-${hashUuid}">
          <stop stop-color="${c1_}" offset="0%"></stop>
          <stop stop-color="${c2}" offset="100%"></stop>
        </linearGradient>
      </defs>
      <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        <rect id="Rectangle" fill="url(#g-${hashUuid})" x="0" y="0" width="80" height="80"></rect>
      </g>
    </svg>`;
}
