...
function refreshMessageList() {
  ...
  if (!chatEventSource) {
    out =
      '<div><i class="fa fa-spinner fa-spin fa-2x"></i> Reconnecting to server...</div>';
    u("#messages").html(out);
    return;
  }
  ...
  chatMessages.forEach(function (msg) {
    u("#messages").append(
      "<hr/><div><span>" +
        ...
        '<button class="btn btn-danger btn-sm float-end remove-msg-btn" data-msg-id="' +
        msg.uuid +
        '">Remove</button></div>'
    );
  });
...
(function () {
  u("#messages").html(
    '<i class="fa fa-spinner fa-spin fa-2x"></i> Connecting to chat server...'
  );
  connectToChatEvents();
})();
